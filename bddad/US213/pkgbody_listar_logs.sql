CREATE OR REPLACE PACKAGE BODY listar_logs AS
  ...
  PROCEDURE pr_ListarLogs IS
    CURSOR all_logs IS
      ...
      FROM LOGS
      ORDER BY data_alteracao, id_log;
  ...
  BEGIN
    ...
    ELSE
      DBMS_OUTPUT.PUT_LINE('Logs ordenadas :');
      ...
      LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('----');
        DBMS_OUTPUT.PUT_LINE('Log id: ' || log_id || '. Utilizador ' || utilizador || ' realizou ' || alteracao || ' na tabela ' || tab || ' a ' || TO_CHAR(data_hora, 'DD/MM/YYYY HH24:MI:SS') || '.');
